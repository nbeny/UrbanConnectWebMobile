                    ...
                    <div className="relative z-[1] h-full flex items-center justify-center">
                        ...
                        <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2">
                            {profiles.map((_, index) => (
                                <div
                                    key={index}
                                    className={`w-2 h-2 rounded-full ${index === currentIndex && slug !== "new" ? "bg-blue-600" : "bg-gray-300"
                                        }`}
                                />
                            ))}
                            <div className={`w-2 h-2 rounded-full ${slug === "new" ? "bg-blue-600" : "bg-gray-300"
                                }`} />
                        </div>
                                ...
                                <div className="grid grid-cols-5 gap-1 md:flex md:space-x-2 md:gap-0">
                                    {menuItems.map((item) => (
                                        <button
                                            ...
                                            className={`px-2 md:px-6 py-2 md:py-3 font-['Manrope:Medium',_sans-serif] text-xs md:text-base transition-colors text-center ${activeTab === item.id
                                                    ...
                                                    <div key={activity.id} className="border-l-4 border-purple-600 bg-gray-50 rounded-r-lg p-4">
                                                        <div className="flex justify-between items-start mb-2">
                                                            ...
                                                            <div className="flex gap-2">
                                                                ...
                                                                <span className={`text-xs px-2 py-1 rounded-full ${activity.status === 'Organisateur' ? 'bg-blue-100 text-blue-700' :
                                        ...
                                        <div className="backdrop-blur-lg bg-white/30 p-4 rounded-xl border border-white/20">
                                            ...
                                            <div className="space-y-2">
                                                <button
                                                    ...
                                                    className={`w-full p-3 text-left rounded-lg transition-colors ${activeContactMenu === "contacts"
                                                            ? "bg-blue-50 border border-blue-200 hover:bg-blue-100"
                                                            : "bg-gray-50 border rounded-lg hover:bg-gray-100"
                                                        }`}
                                                >
                                                    <div className={`font-semibold ${activeContactMenu === "contacts" ? "text-blue-700" : "text-gray-700"
                                                        }`}>Contacts</div>
                                                    <div className={`text-sm ${activeContactMenu === "contacts" ? "text-blue-600" : "text-gray-600"
                                                        }`}>Liste de contacts ({currentProfile.contactsList.length})</div>
                                                </button>
                                                <button
                                                    onClick={() => setActiveContactMenu("blocked")}
                                                    className={`w-full p-3 text-left rounded-lg transition-colors ${activeContactMenu === "blocked"
                                                            ? "bg-red-50 border border-red-200 hover:bg-red-100"
                                                            : "bg-gray-50 border rounded-lg hover:bg-gray-100"
                                                        }`}
                                                >
                                                    <div className={`font-semibold ${activeContactMenu === "blocked" ? "text-red-700" : "text-gray-700"
                                                    ...
                                                    {currentProfile.contactsList.map((contact) => (
                                                        <div key={contact.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                                                            <div className="relative">
                                                                ...
                                                                {/* Status indicator */}
                                                                <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${contact.status === 'En ligne' ? 'bg-green-500' :
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                ...
                                                                <div className="flex items-center gap-2 mt-1">
                                                                    <span className={`text-xs px-2 py-0.5 rounded-full ${contact.type === 'professional' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'
                                                                        }`}>
                                                                        {contact.type === 'professional' ? 'Professionnel' : 'Client'}
                                                                    </span>
                                                                    <span className={`text-xs ${contact.status === 'En ligne' ? 'text-green-600' :
